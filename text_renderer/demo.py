...
output_dir = './mytrain'
save_dir = output_dir# os.path.join(output_dir, flags.tag)
...
corpus = corpus_factory(corpus_mode, flags.chars_file,
...
def generate_img(img_index, q=None):
    ...
    if not flags.viz:
        fname = os.path.join(save_dir, base_name + '.jpg')
        ...
        with lock:
            counter.value += 1
            print_end = '\n' if counter.value == num_img else '\r'
            if counter.value % 100 == 0 or counter.value == num_img:
                print("{}/{} {:2d}%".format(counter.value,
                                            num_img,
                                            int(counter.value / num_img * 100)),
                      end=print_end)
    else:
        utils.viz_img(im)
...
if __name__ == "__main__":
    ...
    tmp_label_path = os.path.join(save_dir, 'tmp_labels.txt')
    ...
    with mp.Pool(processes=get_num_processes(flags)) as pool:
        ...
        pool.starmap(generate_img, zip(
            range(start_index, start_index + num_img), repeat(q)))
